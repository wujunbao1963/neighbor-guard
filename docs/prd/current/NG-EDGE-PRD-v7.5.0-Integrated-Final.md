# NeighborGuard Edge PRD v7.5.0 Integrated (Edge + Gateway Stub)
## 单一真相文档 (Single Source of Truth)

**Version:** 2025-12-20-v7.5.0-EDGE-INTEGRATED  
**Status:** RELEASE  
**Last Updated:** 2025-12-20  
**Normative:** 本文档为 v7.5.0 唯一权威规范（含 Gateway Contract Stub 约束）  
...
# 第五点五部分：Remote Verify & Incident Packet（P0）

本章解决一个关键用户路径：**预警/报警发生时，屋主能立刻“看见现场”，远程核实，再决定是否请求协助/报警/报险**。  
同时把“分散在 Edge + 手机 + 邻居端”的证据与确认，汇总为可审计的 **Incident Packet（事件资料包）**，用于：  
- 用户复盘（当晚睡觉被吵醒/外出度假）  
- 协作（邻居现场查看、代取包裹、补充照片/视频/备注）  
- 对外沟通（报险/维修/与警务沟通话术与要点）

> 重要：Edge 本地只拥有“本地传感器与摄像头证据”。邻居确认/外部取证来自 Cloud/App。  
> 因此 Incident Packet 是 **Edge 部分 + Cloud 部分** 的组合：Edge 负责“第一时间产生可用的视频/时间线”，Cloud 负责“协作证据汇聚与最终建议”。

---

## 5.5.1 Remote Verify 的体验目标（P0）

### AWAY（最危险）
1) **可疑预警（SUSPICION_LIGHT / PRE）**：  
   - 触发后 ≤10s，屋主能打开“Remote Verify View”（近实时视频 + 时间线 + 简要解释）。  
   - 屋主可一键：通知邻居协助观察 / 继续观察 / 忽略（false-positive）。  

2) **周界/破门（SECURITY_HEAVY / PENDING/TRIGGERED）**：  
   - 显示 Entry Delay 倒计时、当前 AlarmSM 状态、最相关摄像头画面（入口点与路径相关）。  
   - 屋主可远程 `disarm/cancel`（有 PIN/生物识别）；也可发起 DCV 协作。  
   - 若邻居提供的最小证据/证人集已满足 Dispatch Readiness：Cloud 允许立即给出 “recommend_call_for_service” 建议（不等待 Edge 收集“第二人确认”，因为 Edge 永远等不到）。

### NIGHT（叫醒优先）
- userAlertLevel 以“叫醒观察”为核心：触发后即时推送并给出“打开视频核实”入口；派警建议由 Readiness/AVS 决定，默认更保守。

### HOME / DISARMED
- Remote Verify 仍可用（例如院子可疑人/包裹），但默认不打扰或轻提示。

---

## 5.5.2 Remote Verify View：视频与时间线（P0）

### 近实时“视频流”的定义（允许两种实现，均视为合规）
- **A) True Live Stream**：WebRTC/RTMP/HLS（需要摄像头或 NG 硬件支持可远程访问/转发）
- **B) Segment Stream（推荐 MVP）**：Edge 以 5–10 秒为粒度持续上传短片段（rolling segments），Cloud 端拼接成“类直播”播放

> 对用户来说 A/B 体验都表现为“正在更新的画面”。  
> 约束：B 必须保证片段间隙 ≤3s（平均），否则视为不可用。

### 视频来源与 Capability Tier
- **Tier V（Video）**：至少 1 路摄像头可提供 A 或 B  
- **Tier E（Entry）**：无可用视频 → Remote Verify View 退化为“传感器时间线 + 解释 + 协作请求”（仍可用，但价值下降）  
- **Tier N（No video）**：必须明确告知用户“无法视频核实”，避免误期望

### 触发策略（Edge）
当事件进入以下阶段，Edge 必须启动 `RemoteMediaCaptureScheduler`：
- `PRE`：抓取 1–2 张快照 + 1 段 5–10s 短片段（如可用）
- `PENDING`：开始 rolling segments（持续 2–5 分钟，或直到 CANCELED）
- `TRIGGERED`：rolling segments 继续；并提高优先级（优先上传入口点相关摄像头）

**上传标签建议：**
- `evidenceType = "remote_segment"`（短片段）
- `evidenceType = "snapshot"`（快照）
- `captureContext = { eventId, workflowClass, cameraId, zoneId, segmentNo }`

---

## 5.5.3 Incident Packet（事件资料包）（P0）

Incident Packet 是一个可审计对象，支持 JSON（机器可用）与可打印摘要（PDF/HTML 可选）。

### 最小内容（P0）
- `eventId` / 时间线（状态转换、关键证据点）
- `workflowClass`、`mode`、`entryPointId`、`zonesInvolved`
- `edgeAssessment`：presence_tier / threat_tier / AVS（local）/ userAlertLevel（local）
- `collabAssessment`：DCV 结果（邻居/亲属确认、ON_SCENE_* 结果）、dispatchReadinessLevel（final）
- `dispatchRecommendation` 与 `dispatch_script_15s`（对外沟通要点）
- `mediaManifest`：
  - rolling segments 清单（或 live stream session info）
  - 邻居上传的照片/视频（外部证据）
  - 关键快照（门窗破损、室内情况）
- `auditTrail`：每一步谁做了什么（含撤回/更改）

### 生成与归属
- **Edge 生成：Edge Snapshot（局部包）**
  - 只包含本地证据与本地状态机时间线
  - 通过 Gateway 上报/同步到 Cloud
- **Cloud 生成：Incident Packet（完整包）**
  - 汇聚 Edge Snapshot + 协作证据 + DCV/Readiness 计算结果
  - 支持后续补充与版本化（追加式）
...
当邻居现场查看后发现“无事”（`ON_SCENE_NO_SIGNS`）：
- 不应直接等同 `CONFIRMED_FALSE`  
- Cloud 端应将建议降级为 `continue_verify`，并记录 `verification_result=ON_SCENE_NO_SIGNS`
- 若随后出现新强证据（例如 door_open + indoor motion），仍可再次升级建议

当邻居发现“现场不安全”（`ON_SCENE_UNSAFE`）：
- 若 AVS/Threat 足够高且策略允许，可给出“条件性建议”（recommend_call_for_service），同时明确提示“不要进入现场”

---

## 5.5.5 对 Gateway Contract 的要求（与 §11 对齐）

为了让 Remote Verify & Incident Packet 在未来 Server 端可落地，Edge→Cloud 事件必须携带以下字段（至少可选）：
- `edge_schema_version`
- `edgeAssessment`（local tiers/avs）
- `remoteVerify`（是否启用、可用视频模式、media manifest 入口）
- `incidentPacket`（edge_snapshot_id 或 manifest stub）
- `eventDisposition`（含 authorized_access / canceled_* / verified_*）
- `auditTrail`（最小：关键状态转换 + disarm/cancel + media upload）
...
- `remote_verify`（mode + media manifest / playback entry；可选但强烈建议）
- `incident_packet_ref`（edge_snapshot_id / packet_id；可选）
- `capabilityTier`
- `edge_schema_version`
  ...
  "events": [
    {
      ...
      "statusTimeline": [
        {"state": "PENDING", "atSec": 0},
        {"state": "TRIGGERED", "atSec": 6}
      ],
      "userAlertLevel": 3,
      ...
      "edgeAssessment": {
        ...
        "presenceTier": 2,
        "threatTier": 3,
        "avs": {"avs_final_level": 2, "avs_peak_level": 3}
      },
      "remoteVerify": {
        "mode": "segment_stream",
        "segmentCadenceSecMax": 10,
        "snapshotsMin": 2,
        "incidentPacketRequired": true
      },
      "incidentPacket": {
        "required": true,
        "edgeSnapshotId": "snap_demo_001"
      },
      "eventDisposition": "open"
    }
  ],
  "evidenceManifest": {
    "eventId": "evt_demo_001",
    "items": [
      {
        ...
        "type": "video",
        "provider": "edge",
        "uri": "evidence://vault/items/evi_snap_001",
        "role": "snapshot",
        "origin": "edge",
        "sequenceNo": 0,
        "createdAt": "2025-12-20T10:30:03Z",
        "captureContext": {"cameraId": "cam_front_door", "zoneId": "zone.entry"}
      },
      {
        "id": "evi_seg_001",
        "type": "video",
        "provider": "edge",
        "uri": "evidence://vault/items/evi_seg_001",
        "role": "remote_segment",
        "origin": "edge",
        "sequenceNo": 1,
        "createdAt": "2025-12-20T10:30:10Z",
        "captureContext": {"cameraId": "cam_front_door", "zoneId": "zone.entry", "segmentNo": 1}
      }
    ]
  },
  "signatures": {
    "eventBundleSha256": "hex...",
    "edgeDeviceId": "edge_demo"
  }
}
```
...
# 附录 A：Drill Suite Schema v2.3.5（对齐 v7.5.0）
...
Drill Suite Schema v2.3.5 对 v7.5.0 的新增断言字段如下（**当 `expected.shouldCreateEvent=true` 时必须提供**）：
...
- 本 PRD 建议 Runner 同时支持 `expected.userAlert` / `expected.dispatchReadiness` 的更细粒度断言（peak/final），但不作为 v2.3.5 的硬必填。
...
*NG Edge PRD v7.5.0 Integrated - Single Source of Truth*
