...
## New Module: RemoteMediaCaptureScheduler (v7.5.0)
- 输入：AlarmSM 状态变化 + entryPoint/zone chain
- 输出：snapshot + rolling remote_segment 上传任务队列
- 约束：网络抖动下优雅降级（降低分辨率/帧率/片段频率），但不得阻塞核心事件引擎
- 安全：证据上传必须签名/哈希；manifest 可审计
