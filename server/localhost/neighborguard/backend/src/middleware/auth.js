...
// ============================================================================
// Require Circle from Header (X-Circle-Id)
// Simpler alternative to requireCircleMember for routes that use header
// ============================================================================
const requireCircle = async (req, res, next) => {
  try {
    const circleId = req.headers['x-circle-id'] || req.query.circleId;
    
    if (!circleId) {
      throw new AppError('Missing Circle ID (set X-Circle-Id header)', 400, 'MISSING_CIRCLE_ID');
    }
    ...
    if (membership.circle.deletedAt) {
      throw new AppError('Circle has been deleted', 404, 'CIRCLE_DELETED');
    }
    ...
    req.home = membership.circle.home;
    req.houseMode = membership.circle.home?.houseMode || 'HOME';
