...
try:
    from .camera_api import camera_router
...
# Include camera router if available
if CAMERA_AVAILABLE:
...
<body class="bg-gray-100 min-h-screen">
        ...
        <div class="bg-white rounded-lg shadow">
            <div class="border-b">
                <nav class="flex">
                    ...
                    <button onclick="showTab('camera')" class="tab-btn px-6 py-3 border-b-2 border-transparent" data-tab="camera">📹 Camera</button>
            ...
            <!-- Camera Tab -->
            <div id="tab-camera" class="tab-content p-6 hidden">
                <h3 class="text-lg font-semibold mb-4">📹 实时摄像头 (仅显示信号状态)</h3>
                
                <div class="bg-yellow-50 border border-yellow-300 rounded p-4 mb-6">
                    <div class="flex items-start gap-2">
                        <span class="text-yellow-600 text-xl">⚠️</span>
                        <div class="text-sm text-yellow-800">
                            <strong>注意:</strong> 此功能仅用于查看摄像头检测的原始信号。<br>
                            暂不发送信号到 Pipeline，不会触发报警或事件。<br>
                            等硬件传感器都配齐后，再进行完整集成测试。
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white border rounded-lg p-4 mb-4">
                    <h4 class="font-semibold mb-3">摄像头配置</h4>
                    <div class="grid grid-cols-3 gap-4 mb-3">
                        <div>
                            <label class="block text-sm text-gray-600 mb-1">摄像头 IP</label>
                            <input type="text" id="cameraIp" value="10.0.0.155" class="w-full border rounded px-3 py-2">
                        </div>
                        <div>
                            <label class="block text-sm text-gray-600 mb-1">检测 FPS</label>
                            <input type="number" id="cameraFps" value="5" min="1" max="30" class="w-full border rounded px-3 py-2">
                        </div>
                        <div>
                            <label class="block text-sm text-gray-600 mb-1">置信度阈值</label>
                            <input type="number" id="cameraThreshold" value="0.6" min="0.1" max="1.0" step="0.1" class="w-full border rounded px-3 py-2">
                        </div>
                    </div>
                    <div class="flex gap-2">
                        <button id="btnStartCamera" onclick="startCamera()" class="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700 font-semibold">
                            ▶ 启动摄像头
                        </button>
                        <button id="btnStopCamera" onclick="stopCamera()" class="px-4 py-2 bg-red-600 text-white rounded hover:bg-red-700 font-semibold hidden">
                            ⏹ 停止摄像头
                        </button>
                    </div>
                    <div id="cameraStatus" class="mt-3 p-2 rounded hidden">
                        <span id="cameraStatusText"></span>
                    </div>
                </div>
                ...
                <div class="bg-white border rounded-lg p-4">
                    <div class="flex justify-between items-center mb-3">
                        <h4 class="font-semibold">检测到的信号 (实时)</h4>
                        <button onclick="clearCameraSignals()" class="px-3 py-1 bg-gray-400 text-white rounded hover:bg-gray-500 text-sm">Clear</button>
                    </div>
                    <div class="text-sm text-gray-500 mb-2">
                        显示最近 50 条检测信号（仅显示，不发送到 Pipeline）
                    </div>
                    <div id="cameraSignalList" class="space-y-2 max-h-96 overflow-y-auto">
                        <div class="text-gray-400 text-center py-8">等待摄像头启动...</div>
                    </div>
                </div>
                ...
                <div class="mt-4 grid grid-cols-4 gap-4">
                    <div class="bg-gray-50 border rounded p-3">
                        <div class="text-sm text-gray-600">总检测次数</div>
                        <div id="cameraStatTotal" class="text-2xl font-bold text-gray-800">0</div>
                    </div>
                    <div class="bg-blue-50 border rounded p-3">
                        <div class="text-sm text-blue-600">Person 检测</div>
                        <div id="cameraStatPerson" class="text-2xl font-bold text-blue-800">0</div>
                    </div>
                    <div class="bg-green-50 border rounded p-3">
                        <div class="text-sm text-green-600">Car 检测</div>
                        <div id="cameraStatCar" class="text-2xl font-bold text-green-800">0</div>
                    </div>
                    <div class="bg-purple-50 border rounded p-3">
                        <div class="text-sm text-purple-600">平均置信度</div>
                        <div id="cameraStatAvgConf" class="text-2xl font-bold text-purple-800">-</div>
                    </div>
                </div>
            </div>
    ...
    <script>
        ...
        let cameraSignals = [];
        let cameraStats = { total: 0, person: 0, car: 0, totalConf: 0 };
        ...
        async function startCamera() {
            ...
            const fps = parseFloat(document.getElementById('cameraFps').value);
            const threshold = parseFloat(document.getElementById('cameraThreshold').value);
            ...
            try {
                ...
                const response = await fetch('/api/camera/start', {
                    ...
                    headers: {'Content-Type': 'application/json'},
                    body: JSON.stringify({
                        zone_id: 'zone_camera_monitor',  // Dummy zone for display only
                        camera_ip: cameraIp,
                        detection_fps: fps,
                ...
                if (response.ok) {
                    ...
                    statusDiv.className = 'mt-3 p-2 bg-green-50 border border-green-300 rounded';
                    statusText.textContent = '🟢 ' + data.message;
                    statusText.className = 'text-green-600 font-semibold';
                    
                    // Clear previous data
                    cameraSignals = [];
                    cameraStats = { total: 0, person: 0, car: 0, totalConf: 0 };
                    updateCameraDisplay();
        ...
        async function stopCamera() {
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    // Stop polling
                    if (window.cameraPollingInterval) {
                        clearInterval(window.cameraPollingInterval);
                        window.cameraPollingInterval = null;
                    }
                } else {
                    btnStop.disabled = false;
                    btnStop.textContent = '⏹ 停止摄像头';
        ...
        function startCameraPolling() {
            // Poll camera status every 500ms
            if (window.cameraPollingInterval) {
                clearInterval(window.cameraPollingInterval);
            }
            
            window.cameraPollingInterval = setInterval(async () => {
                try {
                    ...
                    if (data.active && data.stats) {
                        ...
                        if (data.stats.recent_signals) {
                            data.stats.recent_signals.forEach(sig => {
                                if (!cameraSignals.find(s => s.timestamp === sig.timestamp)) {
                                    cameraSignals.unshift(sig);
                                    
                                    // Update stats
                                    cameraStats.total++;
                                    if (sig.signal_type === 'person_detected') cameraStats.person++;
                                    if (sig.signal_type === 'car_detected') cameraStats.car++;
                                    cameraStats.totalConf += sig.confidence;
                                }
                            });
                            
                            // Keep only last 50 signals
                            if (cameraSignals.length > 50) {
                                cameraSignals = cameraSignals.slice(0, 50);
                            }
        ...
        function updateCameraDisplay() {
            const list = document.getElementById('cameraSignalList');
            
            if (cameraSignals.length === 0) {
                list.innerHTML = '<div class="text-gray-400 text-center py-8">暂无检测信号...</div>';
            } else {
                list.innerHTML = cameraSignals.map(sig => {
                    const typeIcon = sig.signal_type === 'person_detected' ? '🚶' : '🚗';
                    const typeColor = sig.signal_type === 'person_detected' ? 'bg-blue-50 border-blue-200' : 'bg-green-50 border-green-200';
                    const conf = (sig.confidence * 100).toFixed(1);
                    
                    return `
                        <div class="flex items-center justify-between p-2 ${typeColor} border rounded">
                            <div class="flex items-center gap-2">
                                <span class="text-xl">${typeIcon}</span>
                                <div>
                                    <div class="font-medium text-sm">${sig.signal_type}</div>
                                    <div class="text-xs text-gray-500">${new Date(sig.timestamp).toLocaleTimeString()}</div>
                                </div>
                            </div>
                            <div class="text-right">
                                <div class="font-semibold">${conf}%</div>
            ...
            // Update statistics
            document.getElementById('cameraStatTotal').textContent = cameraStats.total;
            document.getElementById('cameraStatPerson').textContent = cameraStats.person;
            document.getElementById('cameraStatCar').textContent = cameraStats.car;
            
            if (cameraStats.total > 0) {
                const avgConf = (cameraStats.totalConf / cameraStats.total * 100).toFixed(1);
                document.getElementById('cameraStatAvgConf').textContent = avgConf + '%';
            }
        }
        
        function clearCameraSignals() {
            cameraSignals = [];
            cameraStats = { total: 0, person: 0, car: 0, totalConf: 0 };
            updateCameraDisplay();
        }
        
        // Populate camera zone selector when zones are loaded
        async function populateCameraZones() {
            // Not needed anymore - camera runs independently
        }
    </script>
</body>
</html>
